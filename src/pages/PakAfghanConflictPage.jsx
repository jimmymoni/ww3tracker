...
const KeyPlayerCard = ({ flag, name, role, objective, side }) => {
  const borderColor = side === 'pakistan' ? 'border-green-500/30' : side === 'afghanistan' ? 'border-red-500/30' : side === 'ttp' ? 'border-red-500/30' : 'border-yellow-500/30';
  const bgColor = side === 'pakistan' ? 'bg-green-500/10' : side === 'afghanistan' ? 'bg-red-500/10' : side === 'ttp' ? 'bg-red-500/10' : 'bg-yellow-500/10';
...
const PakAfghanConflictPage = () => {
  ...
  const stakes = [
    ...
    { icon: <Globe className="w-5 h-5 text-red-400" />, title: 'Afghan Sovereignty', description: 'Taliban legitimacy depends on resisting foreign military operations' },
